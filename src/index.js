...
const createPokemon = (item, likes) => {
  ...
  like.addEventListener("click", async () => {
    ...
    const updatedLike = await getLikes().then((response) => {
      return involvement.likes(response, item.id);
    });
...
const fetchPokemon = async (id) => {
  ...
  const likes = await getLikes().then((response) => {
    return involvement.likes(response, id);
  });
